...
export function NavMain({
    ...
    React.useEffect(() => {
        const saved = localStorage.getItem("nav-main-states")
        if (saved) {
            try {
                setOpenStates(JSON.parse(saved))
            } catch (e) {
                console.error("Failed to parse nav states", e)
            }
        } else {
            // Default to isActive items
    ...
    const toggleItem = (title: string, open: boolean) => {
        const newState = { ...openStates, [title]: open }
        setOpenStates(newState)
        localStorage.setItem("nav-main-states", JSON.stringify(newState))
    }
